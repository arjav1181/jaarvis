...
- JTR-RUN1 (MUST): headless harness runs execute ONLY L0/L1 tools: cwd-pinned to a scratch dir (default `/tmp/jarvis-run`, `JARVIS_RUN_DIR` override, created if missing), hard timeout (default 120s, `JARVIS_RUN_TIMEOUT` override), model pinned (`JARVIS_HARNESS_MODEL`, default `opencode/muse-spark-1.3-contributor-free` — auto-routing flaps), reply capped at 2000 chars. ANY L2-class act (patterns in `shared/destructive.yml`) is REFUSED pre-spawn with a `needs-approval` receipt entry — no pre-approval windows, no exceptions.
